...
function addUsername(list) {
    // We'll use the .forEach() method to add the new value of 'username' to 
    // each developer included in the parameter, 'list'
    list.forEach(x => {
      x.username = x.firstName.toLowerCase() + x.lastName.slice(0, 1).toLowerCase() + (new Date().getFullYear() - x.age);
    });
    return list;
};